...
module randomGenerations =
    ...
    let genRandomArray n = Array.init n (fun _ -> Some (rnd.Next(-25, 25)))
    ...
    let genRandomArray2D x y = Array2D.init x y (fun _ _ -> Some (rnd.Next(-25, 25)))
...
let restoreArray2D (mat: Matrix<_>) = Array2D.init mat.Length1 mat.Length2 (fun i j -> mat[i, j])
    ...
    testList
        ...
        [
            ...
            <| fun _ ->
                ...
                Expect.equal actual_result.Data expected_result.Data "the results were different"
            ...
            <| fun _ ->
                ...
                let vecLen = if rnd.Next(1, 3) = 1 then rnd.Next(1, matLen1) else rnd.Next(matLen1 + 1, 101)

                let mat = genRandomNoneMatrix matLen1 len2
                let vec = genRandomNoneVector vecLen

                Expect.throws (fun _ -> vecMatMultiply vec mat (+) (*) |> ignore) $"The dimensions of the matrix are incompatible
